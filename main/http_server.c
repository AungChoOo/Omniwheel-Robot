...
// Imu sensor data
static esp_err_t http_server_get_imu_readings_json_handler(httpd_req_t *req)
{
	// ESP_LOGI(TAG,"Roll = %.2f Pitch = %.2f Yaw = %.2f ", attitude.r, attitude.p, attitude.y);

	// ESP_LOGI(TAG,"GX = %d GY = %d GZ = %d ", rawData.gx, rawData.gy, rawData.gz);
       
	char rpyJSON[100];

	// sprintf(rpyJSON, "{\"Roll\":\"%.1f\",\"Pitch\":\"%.1f\",\"Yaw\":\"%.1f\"}", attitude.r, attitude.p, attitude.y);
	sprintf(rpyJSON, "{\"X\":\"%d\",\"Y\":\"%d\",\"Z\":\"%d\"}", sensorData.gx, sensorData.gy, sensorData.gz);

	httpd_resp_set_type(req, "application/json");
	httpd_resp_send(req, rpyJSON, strlen(rpyJSON));

	return ESP_OK;
}

// Battery voltage
static esp_err_t http_server_get_battery_readings_json_handler(httpd_req_t *req)
{
	char BattJSON[100];
	
	httpd_resp_set_type(req, "application/json");
	httpd_resp_send(req, BattJSON, strlen(BattJSON));
...

{
    ...
    if (ws_pkt.len) {
		...
		motor.controller = *ws_pkt.payload;
        ESP_LOGI(TAG, "Got packet with message: %s", ws_pkt.payload);
    }
    // ESP_LOGI(TAG, "Packet type: %d", ws_pkt.type);
    if (ws_pkt.type == HTTPD_WS_TYPE_TEXT &&
        strcmp((char*)ws_pkt.payload,"value") == 0) {
        free(buf);
...
static httpd_handle_t http_server_configure(void)
{
	...
	if (httpd_start(&http_server_handle, &config) == ESP_OK)
	{
		...
		// register rpy.json handler
		httpd_uri_t imu_json = {
			.uri = "/rpy.json",
			.method = HTTP_GET,
			.handler = http_server_get_imu_readings_json_handler,
			.user_ctx = NULL
		};
		httpd_register_uri_handler(http_server_handle, &imu_json);

		// register battery.json handler
		// httpd_uri_t batt_json = {
		// 	.uri = "/battery.json",
		// 	.method = HTTP_GET,
		// 	.handler = http_server_get_battery_readings_json_handler,
		// 	.user_ctx = NULL
		// };
		// httpd_register_uri_handler(http_server_handle, &batt_json);

		// register Motor Control handler
		// httpd_uri_t controller_update = {
		// 	.uri = "/controller.json",
		// 	.method = HTTP_POST,
		// 	.handler = http_server_Controller_update_handler,
		// 	.user_ctx = NULL
		// };
		// httpd_register_uri_handler(http_server_handle, &controller_update);
		httpd_uri_t ws = {
			.uri        = "/ws",
			.method     = HTTP_GET,
